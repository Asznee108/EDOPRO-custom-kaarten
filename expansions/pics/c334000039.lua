...
s.listed_names={id}
function s.filter(c)
	return c:IsSetCard(0x123) and c:IsAbleToHand() and not c:IsCode(id)
end
...
function s.repfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x1123) and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) 
...
function s.repop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.GetMatchingGroup(s.rdfilter,tp,LOCATION_MZONE,0,nil)
