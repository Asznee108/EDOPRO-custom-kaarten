...
function s.gyfilter(c,e,tp)
	return c:IsType(TYPE_SYNCHRO) and c:IsLevelBelow(5) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.flipcon2(e,tp,eg,ep,ev,re,r,rp)
  --opd check
	...
	--condition
	return aux.CanActivateSkill(tp) and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_MZONE,0,1,nil,e,tp)
end
function s.flipop2(e,tp,eg,ep,ev,re,r,rp)
  ...
  --opd register
    ...
    if Duel.SpecialSummon(ac,0,tp,tp,true,true,POS_FACEUP)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
        tc=Duel.SelectMatchingCard(tp,s.gyfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp):GetFirst()
