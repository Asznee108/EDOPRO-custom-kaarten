...
function s.initial_effect(c)
	--xyz summon
	Xyz.AddProcedure(c,s.mfilter,8,3,s.ovfilter,aux.Stringid(id,0),3,s.xyzop)
	c:EnableReviveLimit()
	--atk
...
function s.mfilter(c,xyz,sumtype,tp)
	return c:IsRace(RACE_DRAGON,xyz,sumtype,tp)
end
function s.ovfilter(c,tp,lc)
	return c:IsFaceup() and c:IsSummonCode(lc,SUMMON_TYPE_XYZ,tp,0xcf)
end
