...
function s.cfilter(c)
	return c:IsSetCard(0xcf) and c:IsType(TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ+TYPE_LINK+TYPE_RITUAL) and (c:IsFaceup() or not c:IsLocation(LOCATION_MZONE))
end
function s.exfilter(c)
	return c:IsLocation(LOCATION_EXTRA)
end
function s.hafilter(c)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)
	local rg=Duel.GetMatchingGroup(Card.IsAbleToRemove,tp,0,LOCATION_ONFIELD+LOCATION_GRAVE+LOCATION_EXTRA+LOCATION_HAND,nil)
	...
	if g:IsExists(Card.IsType,1,nil,TYPE_XYZ) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		og:Merge(rg:FilterSelect(tp,s.stfilter,1,2,og))
	end
	...
	if g:IsExists(Card.IsType,1,nil,TYPE_RITUAL) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		og:Merge(rg:Filter(s.hafilter,og):RandomSelect(tp,2))
	end
	if #og>0 then
