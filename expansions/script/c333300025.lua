...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc1 and tc2 then
		...
		if (tc1:IsType(TYPE_TUNER) and not tc2:IsType(TYPE_TUNER)) or (tc2:IsType(TYPE_TUNER) and not tc1:IsType(TYPE_TUNER)) then
			local Synchrog=Duel.GetMatchingGroup(s.Synchrofilter,tp,LOCATION_EXTRA,0,nil,g)
			if #Synchrog>0 then
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
				local Synchro=Synchrog:Select(tp,1,1,nil):GetFirst()
				Duel.SynchroSummon(tp,Synchro,nil,g)
			end
		end
	end
	Duel.ShuffleHand(tp)
