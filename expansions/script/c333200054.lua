...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsPlayerCanDraw(tp) and ac:IsControler(tp)
		and ac:IsSetCard(207) and ac:IsRelateToBattle() and ac:CanChainAttack(60) end
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetAttacker()
	if not tc:IsControler(tp) or not tc:IsSetCard(207) or not tc:IsRelateToBattle() or not tc:CanChainAttack(60) then return end
...
function s.caop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetAttacker()
	if tc~=e:GetLabelObject() or not tc:IsControler(tp) or not tc:IsSetCard(207)
